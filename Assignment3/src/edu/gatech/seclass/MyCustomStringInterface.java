...
public interface MyCustomStringInterface {
     ...
     * THOUGHT: remove numbers and empty space , all symbols
     * for each alphabetic letter, create two columns, one for position index (only first one), one for count. 
     * sort count desc first, then sort position asc
     */
    char mostCommonChar();
     ...
     * THOUGHT: for each alphabetic letter, create two columns, one for position index (all indices), one for count. 
     * count = N, if true, keep n<N, if false, keep n>N, use location index to find the character.
     */
